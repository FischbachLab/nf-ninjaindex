...
genomes_combined.into { genomes_combined1; genomes_combined2; genomes_combined3; genomes_combined4 }
/*
*
    STEP 1.1
...
process bowtie2_mapping {
    ...
    //set fna, file ("tmp_*/Sync/paired_fastq/${fna.baseName}.R1.fastq.gz"), file ("tmp_*/Sync/paired_fastq/${fna.baseName}.R2.fastq.gz") into aligned_pair_ch
...
process genome_size {
  ...
  output:
  file "genome.sizes" into gsize_ch

  shell:
  '''
  samtools faidx !{all_genome} && cut -f1,2 !{all_genome}.fai > genome.sizes
  '''
}
*/
bam_ch.into {bam_ch1; bam_ch2}
bai_ch.into {bai_ch1; bai_ch2}

//genomes_ch3.toSortedList().flatten().view()
//bam_ch1.toSortedList().flatten().set{genome_bam_ch}
/*
genomes_ch3
       .concat(bam_ch1)
       .toSortedList() //{file -> file.getBaseName().toString().tokenize('.').get(0)}
       .flatten()
       .buffer(size: 2)
       .set{genome_bam_ch}
...
genomes_ch3
        .concat( bam_ch1 )
        .map { file ->def key = file.getBaseName().toString().tokenize('.').get(0)
...
genome_bam_ch.into {genome_bam_ch1; genome_bam_ch2}

//genome_bam_ch1.view()


/*
   Identify unique portions of a genome, then mask the genome wirh "N"
   cut -f4 !{bam}_filtered.bed | sort | uniq -u > !{bam}_uniq.reads
   cat !{bam}_uniq.reads | parallel grep -m 1 "{}" !{bam}_filtered.bed >> !{bam}_filtered_uniq.bed
*/
process bedtools_processing {
    cpus 4
    tag "$genome"
    publishDir "${params.outdir}/masked_fasta/expected_primary_genomes", mode:'copy', pattern: '*.fna'
		publishDir "${params.outdir}/masked_fasta/expected_primary_bedg", mode:'copy', pattern: '*_expected_primary_genomecov.bedg'
    publishDir "${params.outdir}/masked_fasta/expected_escrow_bedg", mode:'copy', pattern: '*_expected_escrow_genomecov_sorted.bedg'

    input:
    //file g_sizes from gsize_ch
    //file genome from genomes_ch3.toSortedList().flatten()
    //file all_genome from genomes_combined3.collect()
    //file bam from bam_ch1.toSortedList().flatten()
		//file bai from bai_ch1.toSortedList().flatten()
    //set file(genome), file(bam) from genome_bam_ch
    set key, file(genome), file(bam) from genome_bam_ch2

    output:

    file "*expected_escrow_genomecov_sorted.bedg" into bedg_ch
    file "*expected_primary_genomecov.bedg" into bedg_primary_ch
    file "masked_*.fna" into masked_genome_ch

    shell:
    '''
    strain=!{key}
    s=${strain}
    samtools faidx !{genome} && cut -f1,2 !{genome}.fai > ${s}.sizes
    bedtools bamtobed -i !{bam} -tag NM -cigar > ${s}.bed
    cat ${s}.bed | awk '{if( ""($3-$2)"M"==$7 && $5==0){print $0}}' > ${s}_filtered.bed

    awk 'NR==FNR{a[$4]++; next} a[$4]==1' ${s}_filtered.bed ${s}_filtered.bed > ${s}_filtered_uniq.bed

    bedtools genomecov -bg -i ${s}_filtered_uniq.bed -g ${s}.sizes  > ${s}_expected_primary_genomecov.bedg
    bedtools complement -i ${s}_expected_primary_genomecov.bedg -g ${s}.sizes > ${s}_complement_genomecov.bed
    bedtools genomecov -bg -i ${s}_complement_genomecov.bed -g ${s}.sizes | grep  ${s} > ${s}_expected_escrow_genomecov.bedg
    sort -k1,1 -k2,2n ${s}_expected_escrow_genomecov.bedg > ${s}_expected_escrow_genomecov_sorted.bedg
    bedtools maskfasta -fi !{genome} -bed ${s}_expected_escrow_genomecov_sorted.bedg -fo masked_${s}.fna
    '''
}

/*
   Mask all genomes in one file
*/
process mask_genomes {
    cpus 4
		publishDir "${params.outdir}/ninjaIndex/masked_genomes", mode:'copy'
    //saveAs: { filename -> filename.endsWith("expected_primary_genomecov.bedg") ? "expected_primary_bedg/$filename" : filename }


    input:
    file all_genome from genomes_combined4.collect()
    file bedg_files from bedg_ch.toSortedList()

    output:
    file "*masked_expected_primary_genomes.fna"
    file "*expected_escrow_sorted.bedg"

    script:
    """
    bedtools unionbedg -i $bedg_files > ${params.db}_expected_escrow.bedg
    sort -k1,1 -k2,2n ${params.db}_expected_escrow.bedg > ${params.db}_expected_escrow_sorted.bedg
    bedtools maskfasta -fi $all_genome -bed ${params.db}_expected_escrow_sorted.bedg -fo ${params.db}_masked_expected_primary_genomes.fna
    """
  }
...
process generate_Ninja_Index {
  ...
  tag "$bam"
	publishDir "${params.outdir}/ninjaIndex", mode:'copy', saveAs: { filename -> "$filename" }

  input:
  //file 'fasta-dir/*' from masked_genome_ch.toSortedList()
  file 'fasta-dir/*' from genomes_ch4.toSortedList()
  file 'bam-dir/*' from bam_ch2.toSortedList()
  file 'bam-dir/*' from bai_ch2.toSortedList()
